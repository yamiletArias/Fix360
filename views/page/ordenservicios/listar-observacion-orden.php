...
<div class="modal fade" id="modalComponente" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog">
    <form id="formAddSubcategoria" class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Nuevo componente</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">
        <div class="form-floating">
          ...
          <label for="inputSubcategoria" class="form-label"><strong>Componente</strong></label>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-sm btn-secondary" data-bs-dismiss="modal">Cancelar</button>
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Formulario del modal de “Nuevo componente”
    const formModalComponente = document.getElementById('formAddSubcategoria');
    const inputNuevoComp      = document.getElementById('inputSubcategoria');

    // ------------- Segunda opción: agregar sólo la opción nueva -------------
    formModalComponente.addEventListener('submit', async e => {
      e.preventDefault();

      // 1) Obtener el texto ingresado
      const nombreComp = inputNuevoComp.value.trim();
      if (!nombreComp) {
        Swal.fire('Error', 'El nombre del componente no puede quedar vacío', 'warning');
        return;
      }

      try {
        // 2) Enviar la petición AJAX al controller de componentes
        const respuesta = await fetch('http://localhost/fix360/app/controllers/componente.controller.php?task=add', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json; charset=utf-8'
          },
          body: JSON.stringify({ componente: nombreComp })
        });
        const json = await respuesta.json();

        if (!respuesta.ok || json.error) {
          // Si el controlador devolvió "error"
          throw new Error(json.error || 'Error al registrar');
        }

        // 3) Si todo va bien, leer el id y el nombre que regresó el servidor
        const idNuevo   = json.idcomponente;
        const textNuevo = json.componente;

        // 4) Construir un <option> y agregarlo a ambos <select>
        const agregarOption = (selectElem) => {
          const opt = document.createElement('option');
          opt.value       = idNuevo;
          opt.textContent = textNuevo;
          selectElem.appendChild(opt);
          selectElem.value = idNuevo; // seleccionarlo
        };

        agregarOption(selectComp);
        agregarOption(modalSelect);

        // 5) Cerrar el modal y limpiar el input
        Swal.fire('¡Éxito!', 'Componente registrado correctamente', 'success')
          .then(() => {
            const miModal = bootstrap.Modal.getInstance(document.getElementById('modalComponente'));
            miModal.hide();
            inputNuevoComp.value = '';
          });
    ...
    fetch('http://localhost/fix360/app/controllers/componente.controller.php?task=getAll')
      ...
      .catch(err => {
        ...
        selectComp.innerHTML   = '<option value="">Error</option>';
        modalSelect.innerHTML  = '<option value="">Error</option>';
      });
    ...
    if (!idorden) {
      ...
      fetch(`http://localhost/fix360/app/controllers/observacion.controller.php?task=getObservacionByOrden&idorden=${idorden}`)
        ...
        .catch(err => {
          console.error(err);
          cont.innerHTML = `<div class="alert alert-warning" role="alert">No hay observaciones por cargar</div>`;
        });
    }

    // ------------- Registro de nueva observación -------------
    formAdd.addEventListener('submit', async e => {
      ...
      try {
        ...
        Swal.fire('Registrado', json.success, 'success')
    ...
    // ------------- Borrar observación con confirmación -------------
    ...
    // ------------- Editar: abrir modal y precargar datos -------------
    cont.addEventListener('click', e => {
    ...
    // ------------- Submit edición con confirmación -------------
    formEdit.addEventListener('submit', e => {
      ...
      Swal.fire({
        ...
        try {
          ...
          const json = await res.json();
          if (!res.ok || json.error) throw new Error(json.error || res.status);
          Swal.fire('Actualizado', json.success, 'success')
            .then(() => window.location.reload());
        } catch (err) {
          Swal.fire('Error', err.message, 'error');
        }
